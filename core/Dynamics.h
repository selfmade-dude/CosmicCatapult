...
inline Vector2 gravitaionalAccelerationFromBody(const Vector2 &position, const Body &body)
{
    const Vector2 r = position - body.position;
    const double dist = radiusFromPosition(r);
    ...
    const double factor = -body.mu / (dist * dist * dist);
    return Vector2(factor * r.x, factor * r.y);
}
...
inline State2 derivatives(const State2 &state, const std::function<Vector2(const Vector2&)> &accel)
{
    State2 result;

    result.position = state.velocity;

    result.velocity = accel(state.position);
...
inline State2 stepRK4(const State2 &state, double dt, const std::function<Vector2(const Vector2&)> &accel)
{
    const State2 k1 = derivatives(state, accel);

    State2 temp2;
    temp2.position = state.position + k1.position * (dt * 0.5);
    temp2.velocity = state.velocity + k1.velocity * (dt * 0.5);
    const State2 k2 = derivatives(temp2, accel);

    State2 temp3;
    temp3.position = state.position + k2.position * (dt * 0.5);
    temp3.velocity = state.velocity + k2.velocity * (dt * 0.5);
    const State2 k3 = derivatives(temp3, accel);

    State2 temp4;
    temp4.position = state.position + k3.position * dt;
    temp4.velocity = state.velocity + k3.velocity * dt;
    const State2 k4 = derivatives(temp4, accel);
